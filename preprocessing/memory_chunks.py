...
def handle_chunks(args, gen, model, path):
    ...
    while not end_run:
        ...
        for i in range(num_chunks):
            ...
            --------------------------------------------------------- '''
            train_time, io_time = handle_train_chunk(args, training_chunk, gen, model, _round_operation, train_path, i)
            ...
            testing_acc, io_time, end_run, _round_acc = handle_test_chunk(args, training_chunk, testing_chunk, gen, model,
...
def handle_train_chunk(args, training_chunk, gen, model, _round_operation, train_path, itr):
    train_time = 0.0
    io_time = 0.0
    ...
    if _round_operation is 'w':
        ...
        start = time.time()
        tr_C, tr_r = utils.data_generation(training_chunk, gen)
...
def handle_test_chunk(args, training_chunk, testing_chunk, gen, model, _round_operation, test_path, itr, _round_acc, end_run):
    io_time = 0.0
    test_acc = 0.0
    ...
    if _round_operation is 'w':
        ...
        start = time.time()
        ts_C, ts_r = utils.data_generation(int(training_chunk * 0.2), gen)
        end = time.time()
