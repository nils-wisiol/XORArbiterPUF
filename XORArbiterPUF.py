...
def XOR_Breaker(output, args, gen, model, path, path_to_dir_models):
    ...
    for run_index in range(args.runs):
        ...
        if ((args.stages == 64) and (args.streams < 4)) or ((args.stages == 128) and (args.streams < 6)):
            ...
            test_acc, train_acc, train_time = mf.train_test_inMemory(args, gen, model, path)
            if train_acc >= 98.0:
            ...
            ---------------------------------------------------------------------- """
            test_acc, train_time, read_time, write_time = mc.handle_chunks(args, gen, model, path)
